...
<head>
    ...
    <style>
        ...
        .animate-blink-cursor { animation: blink 0.9s step-end infinite; }
        .animate-bounce-soft { animation: bounce-soft 1.8s infinite ease-in-out; }
        .btn-hover { transition: all 0.2s ease; }
        .btn-hover:hover { transform: translateY(-2px); }
        
        .svg-container { position: absolute; inset: 0; width: 100%; height: 100%; pointer-events: none; z-index: 5; }
        .svg-container svg { width: 100%; height: 100%; position: absolute; top: 0; left: 0; object-fit: cover; }
        .safe-top { padding-top: env(safe-area-inset-top); }
        
        /* Primary Font - Space Grotesk for all headings */
        .font-primary, h1, h2, h3, h4, h5, h6, .heading-font {
            font-family: 'Space Grotesk', monospace;
        }
        
        /* Secondary Font - Inter for body text */
        .font-secondary, body, p, span, a, button, input, textarea, label, .body-text {
            font-family: 'Inter', system-ui, sans-serif;
        }
        
        .service-front, .service-back { transition: all 0.5s cubic-bezier(0.25, 0.46, 0.45, 0.94); }
        .service-card-wrapper:hover .service-front { opacity: 0; transform: translateY(-20px); }
        .service-card-wrapper:hover .service-back { opacity: 1; transform: translateY(0); }
        .service-card-wrapper:hover { transform: translateY(-4px); }
        
        .progress-bar { transition: width 1.5s cubic-bezier(0.22, 0.97, 0.36, 1); }
        ...
        .grayscale { filter: grayscale(0.3); transition: filter 0.5s ease; }
        .grayscale:hover { filter: grayscale(0); }
        
        @media (max-width: 640px) { a, button { min-height: 44px; } }
...
<section class="creative-digital-section">
  ...
  <style>
    ...
    .animate-blink { animation: blink 2s ease infinite; }
    .animate-pulse-soft { animation: pulse 2.8s ease infinite; }
    .play-btn .icon-pause { display: none; }
    .video-overlay.playing .play-btn .icon-play { display: none; }
    .video-overlay.playing .play-btn .icon-pause { display: block; }
    .video-overlay.faded { opacity: 0; pointer-events: none; }
    .video-inner:hover .video-overlay.faded.paused { opacity: 1; pointer-events: auto; }
    ...
    @media (max-width: 768px) {
      .video-wrapper {
        padding-bottom: 56.25%; /* Keep 16:9 on mobile too */
      }
    }
  </style>
        ...
        <div class="max-w-7xl mx-auto w-full">
            <div class="flex flex-col lg:flex-row items-center gap-12 md:gap-16 lg:gap-20">
                ...
                <div class="w-full lg:w-1/2 space-y-6">
                    <div class="flex items-center gap-3"><div class="w-8 h-px bg-emerald-400"></div><span class="text-emerald-600 text-xs font-bold tracking-[0.25em] uppercase font-secondary">Since 2014</span></div>
            ...
            <div class="mt-16 sm:mt-20">
                <div class="text-center mb-12">
                    <div class="inline-flex items-center gap-2 mb-4"><i class="fas fa-globe-asia text-emerald-500 text-2xl"></i><h3 class="text-2xl sm:text-3xl font-bold tracking-tight text-gray-800 font-primary">Global Presence</h3></div>
                ...
                <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 max-w-5xl mx-auto">
                    <div class="group relative"><div class="relative bg-white border-2 border-emerald-200 rounded-2xl p-6 text-center transition-all hover:border-emerald-400 hover:shadow-xl overflow-hidden"><div class="absolute inset-0 pointer-events-none"><img src="https://images.unsplash.com/photo-1587474260584-136574528ed5?q=80&w=2070&auto=format&fit=crop" alt="Karachi" class="w-full h-full object-cover opacity-20 group-hover:opacity-30 transition duration-500"></div><div class="relative z-10"><div class="text-5xl mb-3">🇵🇰</div><h4 class="font-bold text-gray-800 text-xl mb-1 font-primary">Pakistan</h4><p class="text-xs font-semibold text-emerald-600 mb-2 font-secondary">HEADQUARTERS</p><p class="text-sm text-gray-600 font-secondary">Est. 2014 · Karachi</p><div class="mt-3 flex justify-center gap-1 text-xs text-gray-400 font-secondary"><span>Financial Hub</span><span>•</span><span>Port City</span></div></div></div></div>
                    <div class="group relative"><div class="relative bg-white border-2 border-emerald-200 rounded-2xl p-6 text-center transition-all hover:border-emerald-400 hover:shadow-xl overflow-hidden"><div class="absolute inset-0 pointer-events-none"><img src="https://images.unsplash.com/photo-1480714378408-67cf0d13bc1b?q=80&w=2070&auto=format&fit=crop" alt="USA" class="w-full h-full object-cover opacity-20 group-hover:opacity-30 transition duration-500"></div><div class="relative z-10"><div class="text-5xl mb-3">🇺🇸</div><h4 class="font-bold text-gray-800 text-xl mb-1 font-primary">United States</h4><p class="text-xs font-semibold text-emerald-600 mb-2 font-secondary">CLIENTS</p><p class="text-sm text-gray-600 font-secondary">Enterprise solutions · Since 2018</p><div class="mt-3 flex justify-center gap-1 text-xs text-gray-400 font-secondary"><span>New York</span><span>•</span><span>California</span><span>•</span><span>Texas</span></div></div></div></div>
                    <div class="group relative"><div class="relative bg-white border-2 border-emerald-200 rounded-2xl p-6 text-center transition-all hover:border-emerald-400 hover:shadow-xl overflow-hidden"><div class="absolute inset-0 pointer-events-none"><img src="https://images.unsplash.com/photo-1517935706615-2717063c2225?q=80&w=2070&auto=format&fit=crop" alt="Canada" class="w-full h-full object-cover opacity-20 group-hover:opacity-30 transition duration-500"></div><div class="relative z-10"><div class="text-5xl mb-3">🇨🇦</div><h4 class="font-bold text-gray-800 text-xl mb-1 font-primary">Canada</h4><p class="text-xs font-semibold text-emerald-600 mb-2 font-secondary">PARTNERS</p><p class="text-sm text-gray-600 font-secondary">Business partnerships · Since 2020</p><div class="mt-3 flex justify-center gap-1 text-xs text-gray-400 font-secondary"><span>Toronto</span><span>•</span><span>Vancouver</span><span>•</span><span>Montreal</span></div></div></div></div>
                </div>
            </div>
        </div>
        <div class="hidden xl:block absolute bottom-10 right-10" aria-hidden="true"><div class="w-16 h-16 bg-emerald-400/20 rotate-12 rounded-2xl absolute -top-4 -right-4"></div><div class="w-16 h-16 bg-gray-900/10 -rotate-6 rounded-2xl absolute top-0 right-0"></div></div>
</section>
...
<section id="services" class="relative py-20 sm:py-28 md:py-32 px-5 sm:px-6 bg-white overflow-hidden">
        ...
        <div class="max-w-7xl mx-auto relative z-10">
            <div class="text-center mb-16 sm:mb-20 md:mb-24">
                <div class="inline-flex items-center gap-2 mb-6"><div class="w-2 h-2 bg-emerald-500 rotate-45"></div><span class="text-xs font-medium tracking-[0.2em] uppercase text-emerald-600 font-secondary">OUR SERVICES</span><div class="w-2 h-2 bg-emerald-500 rotate-45"></div></div>
                <h2 class="font-primary text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold tracking-tight"><span class="text-gray-800">WHAT WE</span><span class="block mt-2 bg-gradient-to-r from-emerald-600 to-teal-500 bg-clip-text text-transparent">EXCEL AT</span></h2>
                <p class="text-base sm:text-lg max-w-xl mx-auto leading-relaxed text-gray-500 mt-4 font-secondary">Comprehensive BPO and digital solutions tailored to your business needs</p>
            </div>

            <!-- BPO SERVICES -->
            <div class="mb-20">
                <div class="flex items-center gap-3 mb-10 sm:mb-12"><div class="w-1 h-8 bg-emerald-500 rounded-full"></div><h3 class="text-2xl sm:text-3xl font-bold text-emerald-600 font-primary">BPO SERVICES</h3><span class="text-sm font-light ml-2 text-emerald-400 font-secondary">/01</span></div>
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
                    <!-- INBOUND CALLS -->
                    <div class="service-card-wrapper group relative"><div class="service-card relative bg-white border-2 border-emerald-200 rounded-2xl h-[420px] sm:h-[460px] overflow-hidden cursor-pointer transition-all duration-500 hover:border-emerald-400"><div class="service-front absolute inset-0 transition-all duration-500 group-hover:opacity-0 group-hover:translate-y-[-20px]"><div class="service-image absolute inset-0 overflow-hidden"><img src="https://images.pexels.com/photos/8867482/pexels-photo-8867482.jpeg?auto=compress&cs=tinysrgb&w=800" alt="Inbound call center" loading="lazy" class="w-full h-full object-cover grayscale transition-transform duration-700 group-hover:scale-110"><div class="absolute inset-0 bg-gradient-to-t from-emerald-700/90 via-emerald-600/60 to-transparent"></div></div><div class="service-content absolute inset-0 p-6 sm:p-8 flex flex-col justify-end z-10"><div class="mb-4"><h4 class="font-primary font-bold text-xl sm:text-2xl text-white mb-3 tracking-wide">INBOUND CALLS</h4><div class="w-12 h-0.5 bg-white/80 rounded-full"></div></div><div class="flex items-center justify-between pt-4 border-t border-white/20"><span class="text-white text-xs tracking-widest font-secondary">24/7 SUPPORT</span><i class="fas fa-arrow-right text-white text-sm opacity-70 group-hover:opacity-100 transition"></i></div></div></div><div class="service-back absolute inset-0 p-6 sm:p-8 opacity-0 translate-y-full group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-500 ease-out bg-gradient-to-br from-emerald-600 to-teal-600"><div class="h-full flex flex-col"><h4 class="font-primary font-bold text-xl sm:text-2xl text-white mb-3">INBOUND CALLS</h4><div class="w-12 h-0.5 bg-white/80 rounded-full mb-4"></div><div class="space-y-3 mb-6"><div class="flex items-center gap-3"><div class="w-1.5 h-1.5 bg-white rounded-full"></div><span class="text-white text-xs sm:text-sm font-secondary">Customer Service</span></div><div class="flex items-center gap-3"><div class="w-1.5 h-1.5 bg-white rounded-full"></div><span class="text-white text-xs sm:text-sm font-secondary">Order Processing</span></div><div class="flex items-center gap-3"><div class="w-1.5 h-1.5 bg-white rounded-full"></div><span class="text-white text-xs sm:text-sm font-secondary">Helpdesk & Troubleshooting</span></div><div class="flex items-center gap-3"><div class="w-1.5 h-1.5 bg-white rounded-full"></div><span class="text-white text-xs sm:text-sm font-secondary">24/7 Call Handling</span></div></div><a href="#" class="mt-auto inline-flex items-center justify-between border border-white/30 rounded-lg px-4 py-2.5 text-white hover:bg-white hover:text-emerald-600 transition-all duration-300 font-secondary"><span class="text-xs sm:text-sm tracking-widest">EXPLORE SERVICE</span><i class="fas fa-arrow-right ml-3"></i></a></div></div></div></div>
                    <!-- OUTBOUND CALLS -->
                    <div class="service-card-wrapper group relative"><div class="service-card relative bg-white border-2 border-emerald-200 rounded-2xl h-[420px] sm:h-[460px] overflow-hidden cursor-pointer transition-all duration-500 hover:border-emerald-400"><div class="service-front absolute inset-0 transition-all duration-500 group-hover:opacity-0 group-hover:translate-y-[-20px]"><div class="service-image absolute inset-0 overflow-hidden"><img src="https://images.unsplash.com/photo-1573497019940-1c28c88b4f3e?q=80&w=1974&auto=format&fit=crop" alt="Outbound calls" loading="lazy" class="w-full h-full object-cover grayscale transition-transform duration-700 group-hover:scale-110"><div class="absolute inset-0 bg-gradient-to-t from-emerald-700/90 via-emerald-600/60 to-transparent"></div></div><div class="service-content absolute inset-0 p-6 sm:p-8 flex flex-col justify-end z-10"><div class="mb-4"><h4 class="font-primary font-bold text-xl sm:text-2xl text-white mb-3 tracking-wide">OUTBOUND CALLS</h4><div class="w-12 h-0.5 bg-white/80 rounded-full"></div></div><div class="flex items-center justify-between pt-4 border-t border-white/20"><span class="text-white text-xs tracking-widests font-secondary">PROACTIVE REACH</span><i class="fas fa-arrow-right text-white text-sm opacity-70 group-hover:opacity-100 transition"></i></div></div></div><div class="service-back absolute inset-0 p-6 sm:p-8 opacity-0 translate-y-full group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-500 ease-out bg-gradient-to-br from-emerald-600 to-teal-600"><div class="h-full flex flex-col"><h4 class="font-primary font-bold text-xl sm:text-2xl text-white mb-3">OUTBOUND CALLS</h4><div class="w-12 h-0.5 bg-white/80 rounded-full mb-4"></div><div class="space-y-3 mb-6"><div class="flex items-center gap-3"><div class="w-1.5 h-1.5 bg-white rounded-full"></div><span class="text-white text-xs sm:text-sm font-secondary">Telemarketing</span></div><div class="flex items-center gap-3"><div class="w-1.5 h-1.5 bg-white rounded-full"></div><span class="text-white text-xs sm:text-sm font-secondary">Appointment Setting</span></div><div class="flex items-center gap-3"><div class="w-1.5 h-1.5 bg-white rounded-full"></div><span class="text-white text-xs sm:text-sm font-secondary">Market Research</span></div><div class="flex items-center gap-3"><div class="w-1.5 h-1.5 bg-white rounded-full"></div><span class="text-white text-xs sm:text-sm font-secondary">Lead Qualification</span></div></div><a href="#" class="mt-auto inline-flex items-center justify-between border border-white/30 rounded-lg px-4 py-2.5 text-white hover:bg-white hover:text-emerald-600 transition-all duration-300 font-secondary"><span class="text-xs sm:text-sm tracking-widest">EXPLORE SERVICE</span><i class="fas fa-arrow-right ml-3"></i></a></div></div></div></div>
                    <!-- CUSTOMER SUPPORT -->
                    <div class="service-card-wrapper group relative"><div class="service-card relative bg-white border-2 border-emerald-200 rounded-2xl h-[420px] sm:h-[460px] overflow-hidden cursor-pointer transition-all duration-500 hover:border-emerald-400"><div class="service-front absolute inset-0 transition-all duration-500 group-hover:opacity-0 group-hover:translate-y-[-20px]"><div class="service-image absolute inset-0 overflow-hidden"><img src="https://images.unsplash.com/photo-1556745757-8d76bdb6984b?q=80&w=1973&auto=format&fit=crop" alt="Customer support" loading="lazy" class="w-full h-full object-cover grayscale transition-transform duration-700 group-hover:scale-110"><div class="absolute inset-0 bg-gradient-to-t from-emerald-700/90 via-emerald-600/60 to-transparent"></div></div><div class="service-content absolute inset-0 p-6 sm:p-8 flex flex-col justify-end z-10"><div class="mb-4"><h4 class="font-primary font-bold text-xl sm:text-2xl text-white mb-3 tracking-wide">CUSTOMER SUPPORT</h4><div class="w-12 h-0.5 bg-white/80 rounded-full"></div></div><div class="flex items-center justify-between pt-4 border-t border-white/20"><span class="text-white text-xs tracking-widest font-secondary">24/7 ASSISTANCE</span><i class="fas fa-arrow-right text-white text-sm opacity-70 group-hover:opacity-100 transition"></i></div></div></div><div class="service-back absolute inset-0 p-6 sm:p-8 opacity-0 translate-y-full group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-500 ease-out bg-gradient-to-br from-emerald-600 to-teal-600"><div class="h-full flex flex-col"><h4 class="font-primary font-bold text-xl sm:text-2xl text-white mb-3">CUSTOMER SUPPORT</h4><div class="w-12 h-0.5 bg-white/80 rounded-full mb-4"></div><div class="space-y-3 mb-6"><div class="flex items-center gap-3"><div class="w-1.5 h-1.5 bg-white rounded-full"></div><span class="text-white text-xs sm:text-sm font-secondary">Multi-channel Support</span></div><div class="flex items-center gap-3"><div class="w-1.5 h-1.5 bg-white rounded-full"></div><span class="text-white text-xs sm:text-sm font-secondary">Feedback Analysis</span></div><div class="flex items-center gap-3"><div class="w-1.5 h-1.5 bg-white rounded-full"></div><span class="text-white text-xs sm:text-sm font-secondary">CRM Management</span></div><div class="flex items-center gap-3"><div class="w-1.5 h-1.5 bg-white rounded-full"></div><span class="text-white text-xs sm:text-sm font-secondary">Complaint Resolution</span></div></div><a href="#" class="mt-auto inline-flex items-center justify-between border border-white/30 rounded-lg px-4 py-2.5 text-white hover:bg-white hover:text-emerald-600 transition-all duration-300 font-secondary"><span class="text-xs sm:text-sm tracking-widest">EXPLORE SERVICE</span><i class="fas fa-arrow-right ml-3"></i></a></div></div></div></div>
                    <!-- TECHNICAL HELPDESK -->
                    <div class="service-card-wrapper group relative"><div class="service-card relative bg-white border-2 border-teal-200 rounded-2xl h-[420px] sm:h-[460px] overflow-hidden cursor-pointer transition-all duration-500 hover:border-teal-400"><div class="service-front absolute inset-0 transition-all duration-500 group-hover:opacity-0 group-hover:translate-y-[-20px]"><div class="service-image absolute inset-0 overflow-hidden"><img src="https://images.unsplash.com/photo-1556745757-8d76bdb6984b?q=80&w=1973&auto=format&fit=crop" alt="Technical helpdesk" loading="lazy" class="w-full h-full object-cover grayscale transition-transform duration-700 group-hover:scale-110"><div class="absolute inset-0 bg-gradient-to-t from-teal-700/90 via-teal-600/60 to-transparent"></div></div><div class="service-content absolute inset-0 p-6 sm:p-8 flex flex-col justify-end z-10"><div class="mb-4"><h4 class="font-primary font-bold text-xl sm:text-2xl text-white mb-3 tracking-wide">TECHNICAL HELPDESK</h4><div class="w-12 h-0.5 bg-white/80 rounded-full"></div></div><div class="flex items-center justify-between pt-4 border-t border-white/20"><span class="text-white text-xs tracking-widest font-secondary">REMOTE IT SUPPORT</span><i class="fas fa-arrow-right text-white text-sm opacity-70 group-hover:opacity-100 transition"></i></div></div></div><div class="service-back absolute inset-0 p-6 sm:p-8 opacity-0 translate-y-full group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-500 ease-out bg-gradient-to-br from-teal-600 to-emerald-600"><div class="h-full flex flex-col"><h4 class="font-primary font-bold text-xl sm:text-2xl text-white mb-3">TECHNICAL HELPDESK</h4><div class="w-12 h-0.5 bg-white/80 rounded-full mb-4"></div><div class="space-y-3 mb-6"><div class="flex items-center gap-3"><div class="w-1.5 h-1.5 bg-white rounded-full"></div><span class="text-white text-xs sm:text-sm font-secondary">Software & Application Support</span></div><div class="flex items-center gap-3"><div class="w-1.5 h-1.5 bg-white rounded-full"></div><span class="text-white text-xs sm:text-sm font-secondary">System & Network Troubleshooting</span></div><div class="flex items-center gap-3"><div class="w-1.5 h-1.5 bg-white rounded-full"></div><span class="text-white text-xs sm:text-sm font-secondary">Remote User Assistance</span></div></div><a href="#" class="mt-auto inline-flex items-center justify-between border border-white/30 rounded-lg px-4 py-2.5 text-white hover:bg-white hover:text-teal-600 transition-all duration-300 font-secondary"><span class="text-xs sm:text-sm tracking-widest">EXPLORE SERVICE</span><i class="fas fa-arrow-right ml-3"></i></a></div></div></div></div>
                </div>
            </div>

            <!-- SALES & LEAD GENERATION -->
            <div class="mb-20">
                <div class="flex items-center gap-3 mb-10 sm:mb-12"><div class="w-1 h-8 bg-emerald-500 rounded-full"></div><h3 class="text-2xl sm:text-3xl font-bold text-emerald-600 font-primary">SALES & LEAD GENERATION</h3><span class="text-sm font-light ml-2 text-emerald-400 font-secondary">/02</span></div>
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
                    <div class="service-card-wrapper group relative"><div class="service-card relative bg-white border-2 border-emerald-200 rounded-2xl h-[420px] sm:h-[460px] overflow-hidden cursor-pointer transition-all duration-500 hover:border-emerald-400"><div class="service-front absolute inset-0 transition-all duration-500 group-hover:opacity-0 group-hover:translate-y-[-20px]"><div class="service-image absolute inset-0 overflow-hidden"><img src="https://images.unsplash.com/photo-1552581234-26160f608093?q=80&w=2070&auto=format&fit=crop" alt="Lead generation" loading="lazy" class="w-full h-full object-cover grayscale transition-transform duration-700 group-hover:scale-110"><div class="absolute inset-0 bg-gradient-to-t from-emerald-700/90 via-emerald-600/60 to-transparent"></div></div><div class="service-content absolute inset-0 p-6 sm:p-8 flex flex-col justify-end z-10"><div class="mb-4"><h4 class="font-primary font-bold text-xl sm:text-2xl text-white mb-3 tracking-wide">LEAD GENERATION</h4><div class="w-12 h-0.5 bg-white/80 rounded-full"></div></div><div class="flex items-center justify-between pt-4 border-t border-white/20"><span class="text-white text-xs tracking-widest font-secondary">QUALIFIED LEADS</span><i class="fas fa-arrow-right text-white text-sm opacity-70 group-hover:opacity-100 transition"></i></div></div></div><div class="service-back absolute inset-0 p-6 sm:p-8 opacity-0 translate-y-full group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-500 ease-out bg-gradient-to-br from-emerald-600 to-teal-600"><div class="h-full flex flex-col"><h4 class="font-primary font-bold text-xl sm:text-2xl text-white mb-3">LEAD GENERATION</h4><div class="w-12 h-0.5 bg-white/80 rounded-full mb-4"></div><div class="space-y-3 mb-6"><div class="flex items-center gap-3"><div class="w-1.5 h-1.5 bg-white rounded-full"></div><span class="text-white text-xs sm:text-sm font-secondary">B2B Prospecting</span></div><div class="flex items-center gap-3"><div class="w-1.5 h-1.5 bg-white rounded-full"></div><span class="text-white text-xs sm:text-sm font-secondary">Cold Calling Campaigns</span></div><div class="flex items-center gap-3"><div class="w-1.5 h-1.5 bg-white rounded-full"></div><span class="text-white text-xs sm:text-sm font-secondary">Appointment Setting</span></div><div class="flex items-center gap-3"><div class="w-1.5 h-1.5 bg-white rounded-full"></div><span class="text-white text-xs sm:text-sm font-secondary">Market Research</span></div></div><a href="#" class="mt-auto inline-flex items-center justify-between border border-white/30 rounded-lg px-4 py-2.5 text-white hover:bg-white hover:text-emerald-600 transition-all duration-300 font-secondary"><span class="text-xs sm:text-sm tracking-widest">EXPLORE SERVICE</span><i class="fas fa-arrow-right ml-3"></i></a></div></div></div></div>
                </div>
            </div>

            <!-- WEB SOLUTIONS -->
            <div class="mb-20">
                <div class="flex items-center gap-3 mb-10 sm:mb-12"><div class="w-1 h-8 bg-teal-500 rounded-full"></div><h3 class="text-2xl sm:text-3xl font-bold text-teal-600 font-primary">WEB SOLUTIONS</h3><span class="text-sm font-light ml-2 text-teal-400 font-secondary">/03</span></div>
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
                    <div class="service-card-wrapper group relative"><div class="service-card relative bg-white border-2 border-teal-200 rounded-2xl h-[420px] sm:h-[460px] overflow-hidden cursor-pointer transition-all duration-500 hover:border-teal-400"><div class="service-front absolute inset-0 transition-all duration-500 group-hover:opacity-0 group-hover:translate-y-[-20px]"><div class="service-image absolute inset-0 overflow-hidden"><img src="https://images.unsplash.com/photo-1467232004584-a241de8bcf5d?q=80&w=2069&auto=format&fit=crop" alt="Web development" loading="lazy" class="w-full h-full object-cover grayscale transition-transform duration-700 group-hover:scale-110"><div class="absolute inset-0 bg-gradient-to-t from-teal-700/90 via-teal-600/60 to-transparent"></div></div><div class="service-content absolute inset-0 p-6 sm:p-8 flex flex-col justify-end z-10"><div class="mb-4"><h4 class="font-primary font-bold text-xl sm:text-2xl text-white mb-3 tracking-wide">WEB DEVELOPMENT</h4><div class="w-12 h-0.5 bg-white/80 rounded-full"></div></div><div class="flex items-center justify-between pt-4 border-t border-white/20"><span class="text-white text-xs tracking-widest font-secondary">DESIGN & DEVELOP</span><i class="fas fa-arrow-right text-white text-sm opacity-70 group-hover:opacity-100 transition"></i></div></div></div><div class="service-back absolute inset-0 p-6 sm:p-8 opacity-0 translate-y-full group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-500 ease-out bg-gradient-to-br from-teal-600 to-emerald-600"><div class="h-full flex flex-col"><h4 class="font-primary font-bold text-xl sm:text-2xl text-white mb-3">WEB DEVELOPMENT</h4><div class="w-12 h-0.5 bg-white/80 rounded-full mb-4"></div><div class="space-y-3 mb-6"><div class="flex items-center gap-3"><div class="w-1.5 h-1.5 bg-white rounded-full"></div><span class="text-white text-xs sm:text-sm font-secondary">Custom Web Development</span></div><div class="flex items-center gap-3"><div class="w-1.5 h-1.5 bg-white rounded-full"></div><span class="text-white text-xs sm:text-sm font-secondary">E-commerce Solutions</span></div><div class="flex items-center gap-3"><div class="w-1.5 h-1.5 bg-white rounded-full"></div><span class="text-white text-xs sm:text-sm font-secondary">API Integration</span></div><div class="flex items-center gap-3"><div class="w-1.5 h-1.5 bg-white rounded-full"></div><span class="text-white text-xs sm:text-sm font-secondary">Responsive Design</span></div></div><a href="#" class="mt-auto inline-flex items-center justify-between border border-white/30 rounded-lg px-4 py-2.5 text-white hover:bg-white hover:text-teal-600 transition-all duration-300 font-secondary"><span class="text-xs sm:text-sm tracking-widest">EXPLORE SERVICE</span><i class="fas fa-arrow-right ml-3"></i></a></div></div></div></div>
                </div>
            </div>

            <!-- DIGITAL MARKETING -->
            <div class="mb-20">
                <div class="flex items-center gap-3 mb-10 sm:mb-12"><div class="w-1 h-8 bg-teal-500 rounded-full"></div><h3 class="text-2xl sm:text-3xl font-bold text-teal-600 font-primary">DIGITAL MARKETING</h3><span class="text-sm font-light ml-2 text-teal-400 font-secondary">/04</span></div>
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
                    <div class="service-card-wrapper group relative"><div class="service-card relative bg-white border-2 border-teal-200 rounded-2xl h-[420px] sm:h-[460px] overflow-hidden cursor-pointer transition-all duration-500 hover:border-teal-400"><div class="service-front absolute inset-0 transition-all duration-500 group-hover:opacity-0 group-hover:translate-y-[-20px]"><div class="service-image absolute inset-0 overflow-hidden"><img src="https://images.unsplash.com/photo-1551288049-bebda4e38f71?q=80&w=2070&auto=format&fit=crop" alt="SEO & SEM" loading="lazy" class="w-full h-full object-cover grayscale transition-transform duration-700 group-hover:scale-110"><div class="absolute inset-0 bg-gradient-to-t from-teal-700/90 via-teal-600/60 to-transparent"></div></div><div class="service-content absolute inset-0 p-6 sm:p-8 flex flex-col justify-end z-10"><div class="mb-4"><h4 class="font-primary font-bold text-xl sm:text-2xl text-white mb-3 tracking-wide">SEO & SEM</h4><div class="w-12 h-0.5 bg-white/80 rounded-full"></div></div><div class="flex items-center justify-between pt-4 border-t border-white/20"><span class="text-white text-xs tracking-widest font-secondary">CAMPAIGN MANAGEMENT</span><i class="fas fa-arrow-right text-white text-sm opacity-70 group-hover:opacity-100 transition"></i></div></div></div><div class="service-back absolute inset-0 p-6 sm:p-8 opacity-0 translate-y-full group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-500 ease-out bg-gradient-to-br from-teal-600 to-emerald-600"><div class="h-full flex flex-col"><h4 class="font-primary font-bold text-xl sm:text-2xl text-white mb-3">SEO & SEM</h4><div class="w-12 h-0.5 bg-white/80 rounded-full mb-4"></div><div class="space-y-3 mb-6"><div class="flex items-center gap-3"><div class="w-1.5 h-1.5 bg-white rounded-full"></div><span class="text-white text-xs sm:text-sm font-secondary">Keyword Research & Strategy</span></div><div class="flex items-center gap-3"><div class="w-1.5 h-1.5 bg-white rounded-full"></div><span class="text-white text-xs sm:text-sm font-secondary">PPC Campaign Setup</span></div><div class="flex items-center gap-3"><div class="w-1.5 h-1.5 bg-white rounded-full"></div><span class="text-white text-xs sm:text-sm font-secondary">Performance Analytics</span></div><div class="flex items-center gap-3"><div class="w-1.5 h-1.5 bg-white rounded-full"></div><span class="text-white text-xs sm:text-sm font-secondary">Conversion Optimization</span></div></div><a href="#" class="mt-auto inline-flex items-center justify-between border border-white/30 rounded-lg px-4 py-2.5 text-white hover:bg-white hover:text-teal-600 transition-all duration-300 font-secondary"><span class="text-xs sm:text-sm tracking-widest">EXPLORE SERVICE</span><i class="fas fa-arrow-right ml-3"></i></a></div></div></div></div>
                    <div class="service-card-wrapper group relative"><div class="service-card relative bg-white border-2 border-teal-200 rounded-2xl h-[420px] sm:h-[460px] overflow-hidden cursor-pointer transition-all duration-500 hover:border-teal-400"><div class="service-front absolute inset-0 transition-all duration-500 group-hover:opacity-0 group-hover:translate-y-[-20px]"><div class="service-image absolute inset-0 overflow-hidden"><img src="https://images.unsplash.com/photo-1611926653458-09294b3142bf?q=80&w=2070&auto=format&fit=crop" alt="Social Media" loading="lazy" class="w-full h-full object-cover grayscale transition-transform duration-700 group-hover:scale-110"><div class="absolute inset-0 bg-gradient-to-t from-teal-700/90 via-teal-600/60 to-transparent"></div></div><div class="service-content absolute inset-0 p-6 sm:p-8 flex flex-col justify-end z-10"><div class="mb-4"><h4 class="font-primary font-bold text-xl sm:text-2xl text-white mb-3 tracking-wide">SOCIAL MEDIA</h4><div class="w-12 h-0.5 bg-white/80 rounded-full"></div></div><div class="flex items-center justify-between pt-4 border-t border-white/20"><span class="text-white text-xs tracking-widest font-secondary">MANAGEMENT</span><i class="fas fa-arrow-right text-white text-sm opacity-70 group-hover:opacity-100 transition"></i></div></div></div><div class="service-back absolute inset-0 p-6 sm:p-8 opacity-0 translate-y-full group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-500 ease-out bg-gradient-to-br from-teal-600 to-emerald-600"><div class="h-full flex flex-col"><h4 class="font-primary font-bold text-xl sm:text-2xl text-white mb-3">SOCIAL MEDIA MANAGEMENT</h4><div class="w-12 h-0.5 bg-white/80 rounded-full mb-4"></div><div class="space-y-3 mb-6"><div class="flex items-center gap-3"><div class="w-1.5 h-1.5 bg-white rounded-full"></div><span class="text-white text-xs sm:text-sm font-secondary">Multi-Platform Management</span></div><div class="flex items-center gap-3"><div class="w-1.5 h-1.5 bg-white rounded-full"></div><span class="text-white text-xs sm:text-sm font-secondary">Content Calendar Planning</span></div><div class="flex items-center gap-3"><div class="w-1.5 h-1.5 bg-white rounded-full"></div><span class="text-white text-xs sm:text-sm font-secondary">Community Engagement</span></div><div class="flex items-center gap-3"><div class="w-1.5 h-1.5 bg-white rounded-full"></div><span class="text-white text-xs sm:text-sm font-secondary">Analytics & Insights</span></div></div><a href="#" class="mt-auto inline-flex items-center justify-between border border-white/30 rounded-lg px-4 py-2.5 text-white hover:bg-white hover:text-teal-600 transition-all duration-300 font-secondary"><span class="text-xs sm:text-sm tracking-widest">EXPLORE SERVICE</span><i class="fas fa-arrow-right ml-3"></i></a></div></div></div></div>
                </div>
            </div>
            ...
            <div class="mt-16 sm:mt-20 md:mt-24 text-center">
                <div class="max-w-md mx-auto">
                    ...
                    <a href="#contact" class="group inline-flex items-center gap-3 px-6 sm:px-8 py-3 sm:py-4 font-semibold rounded-full bg-gradient-to-r from-emerald-500 to-teal-500 text-white hover:shadow-xl hover:scale-[1.02] transition-all font-secondary"><span>GET IN TOUCH</span><i class="fas fa-arrow-right text-sm group-hover:translate-x-1 transition"></i></a>
        ...
        <div class="max-w-7xl mx-auto relative z-10 w-full">
            ...
            <div class="mt-16 sm:mt-20 md:mt-24 pt-8 sm:pt-12 border-t-2 border-emerald-200">
                ...
                <div class="relative overflow-hidden w-full">
                    ...
                    <!-- Navigation Buttons -->
                    <div class="flex items-center justify-center mt-8 sm:mt-10 md:mt-12 gap-4 sm:gap-6">
...
<?php @include("footer.php") ?>
    ...
    <script>
        (function () {
            ...
            let index = 0, charIndex = 0, currentPhrase = phrases[0], isDeleting = false;
            const taglineElement = document.getElementById('dynamicTagline');
            if (!taglineElement) return;
            function typeEffect() {
                const fullText = phrases[index];
                if (isDeleting) { currentPhrase = fullText.substring(0, charIndex - 1); charIndex--; }
                else { currentPhrase = fullText.substring(0, charIndex + 1); charIndex++; }
                taglineElement.textContent = currentPhrase;
                if (!isDeleting && charIndex === fullText.length) { isDeleting = true; setTimeout(typeEffect, 2000); return; }
                if (isDeleting && charIndex === 0) { isDeleting = false; index = (index + 1) % phrases.length; setTimeout(typeEffect, 500); return; }
        ...
        if (nextBtn) nextBtn.addEventListener('click', () => { currentIndex = (currentIndex + 1) % totalSlides; updateCarousel(); });
        if (prevBtn) prevBtn.addEventListener('click', () => { currentIndex = (currentIndex - 1 + totalSlides) % totalSlides; updateCarousel(); });
        indicators.forEach((dot, index) => { dot.addEventListener('click', () => { currentIndex = index; updateCarousel(); }); });
            ...
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    ...
                    counters.forEach(counter => {
                        ...
                        const updateCounter = () => {
                            current += increment;
                            if (current < target) { counter.innerText = Math.floor(current) + suffix; requestAnimationFrame(updateCounter); }
